...
namespace ShockwaveGORN.Managers
{
    internal static class M_Enemy
    {
            ...
            switch (damageType)
            {
                ...
                case DamageType.Blunt:
                    if (fist.caestusType == CaestusType.None)
                        M_HapticFeedback.fist.OnBlunt(velocity, fist.left);
                    else if(M_HapticFeedback.WeaponsByCaestusType.TryGetValue(fist.caestusType, out I_WeaponBase weapon_blunt))
                        weapon_blunt.OnBlunt(velocity, fist.left);
                    goto default;

                case DamageType.Stab:
                    if ((fist.caestusType != CaestusType.None)
                        && M_HapticFeedback.WeaponsByCaestusType.TryGetValue(fist.caestusType, out I_WeaponBase weapon_stab))
                        weapon_stab.OnStab(velocity, fist.left);
                    goto default; 

                case DamageType.Cut:
                    if ((fist.caestusType != CaestusType.None)
                        && M_HapticFeedback.WeaponsByCaestusType.TryGetValue(fist.caestusType, out I_WeaponBase weapon_cut))
        ...
        internal static void OnWeaponDamage(WeaponBase weaponBase, Vector3 velocity, DamageType damageType, Collision collision)
        {
            ...
            if (!M_HapticFeedback.WeaponsByType.TryGetValue(weaponType, out I_WeaponBase weapon))
                return;
